 ...
 */
const fix = <T, U>(
    fn: (current: T, next: (completed: U) => void) => Promise<U>| void) =>
    (promise) => {
      /**
      ...
      let finalResolver;
      let rejecter;
      let resolver: ((c: T) => void)|null = (v) => {
        try {
          let completeCalled = false;
          let completeValue;
          let complete: ((vComplete: U) => void)|null = (vComplete: U) => {
            completeCalled = true;
          ...
          if (completeCalled) {
            ...
            nextPromise!.then(resolver as any, rejecter as any);
          }
          complete = null;
        } catch (err) {
          rejecter(err);
